...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $username = $_SESSION['username'] ?? 'system'; // fallback

    try {
        $pdo->beginTransaction();

        // Insert into sales (for pickup)
        ...
        foreach ($order['items'] as $item) {
            ...
            $checkStmt = $pdo->prepare("SELECT id, quantity FROM waiting_list WHERE client_id = :client_id AND product = :product LIMIT 1");
            $checkStmt->execute([':client_id' => $clientId, ':product' => $product]);

            if ($existing = $checkStmt->fetch(PDO::FETCH_ASSOC)) {
                $newQty = $existing['quantity'] + $quantity;
                $update = $pdo->prepare("UPDATE waiting_list SET quantity = :quantity, total = :total WHERE id = :id");
                $update->execute([
                    ':quantity' => $newQty,
                    ':total' => $newQty * $price,
                    ':id' => $existing['id']
            ...
            // Insert into sale_items (for pickup, no stock update)
        ...
        $message = urlencode("The goods have been added to $clientName's tab.");
